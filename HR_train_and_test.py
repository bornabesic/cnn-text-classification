...
tf.flags.DEFINE_integer("BATCH_SIZE", 1, "Training batch size")
...
tf.flags.DEFINE_integer("VECTOR_DIM", 300, "Word vector dimension")
...
train, test, num_classes, class_dict, max_sentence_length = HR_data.load_dataset(FLAGS.DATASET)
...
HR_vocabulary.load_indices(FLAGS.DATASET)
...
logger.log("Vocabulary size: " + str(HR_vocabulary.vocabulary_size))
...
with tf.Session(config=config) as sess, logger:
	...
	neural_network = model_class(
		...
		vocabulary_size=HR_vocabulary.vocabulary_size,
